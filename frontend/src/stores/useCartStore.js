...
export const useCartStore = create((set, get) => ({
  ...
  addBundleToCart: async (bundle) => {
    try {
      ...
      set((prevState) => {
        const existingBundle = prevState.cart.find((item) => item.type === 'bundle' && item._id === bundle._id)
        if (existingBundle) {
          return {
            cart: prevState.cart.map((item) =>
              item.type === 'bundle' && item._id === bundle._id ? { ...item, quantity: item.quantity + 1 } : item
            )
          }
        } else {
          return {
  ...
  updateQuantity: async (id, quantity, type) => {
    ...
    try {
      ...
      set((prevState) => ({
        cart: prevState.cart.map((item) => (item._id === id && item.type === type ? { ...item, quantity } : item))
      }))
      get().calculateTotals()
    } catch (error) {
