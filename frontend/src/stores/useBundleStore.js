...
export const useBundleStore = create((set) => ({
  ...
  createBundle: async (bundle) => {
    ...
    try {
      ...
      set((state) => ({
        bundles: [...state.bundles, res.data],
        loading: false,
      }));
      return { success: true };
    } catch (error) {
      set({
        error: error.response?.data?.message || 'Failed to create bundle',
        loading: false,
      });
      return {
        success: false,
  ...
  deleteBundle: async (id) => {
    ...
    try {
      ...
      set((state) => ({
        bundles: state.bundles.filter((b) => b._id !== id),
        loading: false,
      }));
    } catch (error) {
      set({
        error: error.response?.data?.message || 'Failed to delete bundle',
        loading: false,
      });
    }
  },
